...
export class OrderRepository extends PgRepository implements Setup {
  ...
  async create ({ accountId, products, total }: AddOrderRepository.Input): Promise<AddOrderRepository.Output> {
    const repository = this.getRepository(Order)

    await repository.save({ id: this.uuid.generate(), accountId, products, total })
  }
