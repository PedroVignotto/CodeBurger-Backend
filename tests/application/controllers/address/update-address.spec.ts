...
describe('UpdateAddressController', () => {
  ...
  it('Should call updateAddress with correct values', async () => {
    await sut.handle({ accountId, id, surname, number, complement })

    expect(updateAddress).toHaveBeenCalledWith({ accountId, id, surname, number, complement })
  ...
  it('Should return badRequest if updateAddress return NonExistentFieldError', async () => {
    ...
    const { statusCode, data } = await sut.handle({ accountId, id, surname, number, complement })
  ...
  it('Should return noContent if valid data is provided', async () => {
    const { statusCode } = await sut.handle({ accountId, id, surname, number, complement })
